...
class WaveUNetPP(nn.Module):
    def __init__(self):
        super(WaveUNetPP, self).__init__()
        self.L = params["L"]
        self.K = params["K"]
        self.Fc = params["Fc"]
        self.fd = params["fd"]
        ...
        for i in range(self.L):
                ...
                self.up_convs_2.append(nn.Conv1d(in_channels= downsample_inchannel + upsample_inchannel, out_channels= self.Fc * (i+1), kernel_size=self.fu, padding=self.fu//2))
        ...
        self.mask_conv = nn.Conv1d(in_channels=self.Fc + 1, out_channels=self.K-1, kernel_size=1)
    ...
    def forward(self, x):
        ...
        up_pipe2 = [down_pipe[-1]]
        for i in range(L):
                tmp = torch.cat((down_pipe[L - i], F.interpolate(up_pipe2[-1], scale_factor=2.0, mode='linear')), dim=1)
                up_pipe2.append(F.relu(self.up_convs_2[L - i - 1](tmp)))

        tmp = torch.cat((x, up_pipe[-1]), dim=1)
        tmp2 = torch.cat((x, up_pipe2[-1]), dim=1)
        out_k_minus_1 = torch.tanh(self.final_conv(tmp)) * torch.sigmoid(self.mask_conv(tmp2))
